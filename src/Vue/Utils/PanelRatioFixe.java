...
public class PanelRatioFixe extends JPanel {
    ...
    public static void main(String[] args) {
        SwingUtilities.invokeLater(() -> {
            ...
            panelRatio169.setBackground(Color.LIGHT_GRAY);
            panelRatio169.setOpaque(true); // Important pour voir le fond du PanelRatioFixe

            JPanel contenuCarrePanel = new JPanel(new GridBagLayout()); // Pour centrer le label dans le panel carré
            contenuCarrePanel.setBackground(Color.CYAN);
            contenuCarrePanel.setPreferredSize(new Dimension(150,150));
            JLabel labelCarre = new JLabel("Carré (1:1)");
            contenuCarrePanel.add(labelCarre);


            PanelRatioFixe panelCarre = new PanelRatioFixe(contenuCarrePanel);
            panelCarre.setBorder(BorderFactory.createTitledBorder("Panneau Carré (1:1)"));
            panelCarre.setBackground(Color.DARK_GRAY); // Couleur de fond pour voir ses limites
            panelCarre.setOpaque(true); // Important pour le test visuel


            JSplitPane splitPane = new JSplitPane(JSplitPane.HORIZONTAL_SPLIT, panelRatio169, panelCarre);
            splitPane.setResizeWeight(0.5);
            // Pour améliorer la performance du redimensionnement du JSplitPane lui-même:
            splitPane.setContinuousLayout(false); // ou true, tester ce qui est mieux. False attend la fin du drag.
